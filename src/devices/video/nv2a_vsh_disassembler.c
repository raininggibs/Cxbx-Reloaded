...
static inline uint32_t parse_mac_opcode(const uint32_t *token) {
  return EXTRACT(token, 1, 21, 4);
}

static inline uint32_t parse_ilu_opcode(const uint32_t *token) {
...
static inline uint32_t parse_b_type(const uint32_t *token) {
...
static inline uint32_t parse_a_type(const uint32_t *token) {
...
static inline uint32_t parse_c_type(const uint32_t *token) {
...
static Nv2aVshParseResult parse_outputs(Nv2aVshStep *out,
  ...
  if (temp_writemask_mac) {
    ...
    out->mac.outputs[0].writemask = (Nv2aVshWritemask)temp_writemask_mac;
  }
  ...
  if (temp_writemask_ilu) {
    ...
    out->ilu.outputs[0].writemask = (Nv2aVshWritemask)temp_writemask_ilu;
  }
  ...
  if (output_writemask) {
    ...
    output->writemask = (Nv2aVshWritemask)output_writemask;
  }
  ...
  if (out->mac.opcode == NV2AOP_ARL) {
    ...
    out->mac.outputs[0].writemask = (Nv2aVshWritemask)0;
  }
