...
// ******************************************************************
xbox::hresult_xt WINAPI xbox::EMUPATCH(D3DDevice_BeginPush)(dword_xt Count)
{
...
// For XDK before 4531
// ******************************************************************
xbox::hresult_xt WINAPI xbox::EMUPATCH(D3DDevice_BeginPush2)(dword_xt Count, dword_xt** ppPush)
{
	LOG_FUNC_BEGIN
		LOG_FUNC_ARG(Count)
		LOG_FUNC_ARG(ppPush)
...
void EmuKickOff(void)
{
	...
	__asm {
		//XB_TRAMPOLINE_CDevice_KickOff() require D3DDEVICE in ecx as this pointer.
...
xbox::dword_xt* WINAPI xbox::EMUPATCH(D3DDevice_EndPush)(dword_xt *pPush)
{
	...
	//in D3DDevice_EndPush() before it returns, it has g_pDevice in ECX, which we need it to pass to XB_TRMP(CDevice_KickOff)() as this pointer.
	//DWORD * Xbox_D3DDevice;
	//__asm {
...
)
{
    ...
    __asm {
        LTCG_PROLOGUE
        mov  Mode, eax
		call XB_TRMP(D3DDevice_SetShaderConstantMode_0)
    }
...
)
{
	...
	if (is_pushbuffer_recording()) {
		XB_TRMP(D3DDevice_DrawVertices_4)(PrimitiveType);
	}
	// We can't call emupatch here because tarte function calls trampoline which might not be available in guest code.
...
xbox::void_xt WINAPI CxbxImpl_DrawVerticesUP
(
	xbox::X_D3DPRIMITIVETYPE  PrimitiveType,
	xbox::uint_xt             VertexCount,
...
)
{
	...
	CxbxImpl_DrawVerticesUP(PrimitiveType, VertexCount, pVertexStreamZeroData, VertexStreamZeroStride);
...
)
{
	...
	if (is_pushbuffer_recording()) {
		XB_TRMP(D3DDevice_DrawVerticesUP_12)(PrimitiveType, VertexCount, VertexStreamZeroStride);
	}
